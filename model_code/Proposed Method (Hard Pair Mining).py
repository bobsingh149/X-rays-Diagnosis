...
for l in labels:   
    ...
    print(l)
    df_train, df_test = train_test_split(char_imgs, test_size=0.30, random_state=42)
...
class Data_Generator(object):
    ...
    def get_hard_pair(self,df1,df2,same=True):
        ...
        if same:
            hard_idx=np.argmin(pred)
        else:
            hard_idx=np.argmax(pred)
...
train_gen = Data_Generator(df_X_train, image_shape, batch_size)
...
generator = Data_Generator(df_X_train, image_shape, batch_size).get_batch()
validator = Data_Generator(df_X_train, image_shape, batch_size).get_batch()
...
class N_way_validate(object):
    ...
    def __init__(self, df, image_shape, n=20, k_trial=30):
        # Prepare parameters
        self.df = df.copy()
        self.h, self.w, self.c = image_shape
        self.labels = list(set(self.df['Class']))
    ...
    def score(self, model):
        ...
        res = np.round((score / self.k_trial) * 100, 3)
...
def get_hard_pair(df1,df2,same=True):
     ...
     size = n1*n2
     temp_pairs = [np.zeros((size, 100, 100, 3)) for i in range(2)]
